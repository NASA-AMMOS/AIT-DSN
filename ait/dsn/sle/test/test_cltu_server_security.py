...
@pytest.mark.parametrize("test_udp_host,test_port,test_rest", [
    ('127.0.0.1', 9010, 7661),
    ('localhost', 9011, 7662),
])
...
def test_udp_host_can_bind_to_non_loopback_when_explicitly_configured():
    ...
    try:
        ...
        assert bound_host == '0.0.0.0', \
            f"Expected binding to 0.0.0.0, got {bound_host}"

    finally:
        servers.kill_servers()


@pytest.mark.parametrize("non_loopback_host", [
    '192.168.1.100',
    '10.0.0.1',
    '172.16.0.1',
    '0.0.0.0',
...
def test_non_loopback_udp_host_configurations_work(non_loopback_host):
    ...
    try:
        ...
        assert bound_host in [non_loopback_host, '0.0.0.0'], \
...
def test_udp_host_independent_from_rest_host():
    ...
    try:
        ...
        assert bound_udp_host == '127.0.0.1', \
