...
def test_TCTF_no_ecf(random_payload_no_ecf):
    ...
    assert (
        decoded_frame_header[tctf.HeaderKeys.TRANSFER_FRAME_VERSION_NUM]
        == tf_version_num
    )
    ...
    assert (
        decoded_frame_header[tctf.HeaderKeys.FRAME_LENGTH]
        == tctf.ICD.Sizes.MAX_FRAME_OCTETS.value - 1
    )  # Per ICD subtract 1
...
def test_TCTF_random_len_ecf(random_payload_random_len):
    ...
    assert (
        decoded_frame_header[tctf.HeaderKeys.FRAME_LENGTH]
        == 71
        + tctf.ICD.Sizes.PRIMARY_HEADER_OCTETS.value
        + tctf.ICD.Sizes.ECF_OCTETS.value
        - 1
    )  # Per ICD subtract 1
    assert decoded_frame_header[tctf.HeaderKeys.FRAME_SEQ_NUM] == frame_seq_num
...
def test_TCTF_random_len_no_ecf(random_payload_random_len):
    ...
    assert (
        decoded_frame_header[tctf.HeaderKeys.FRAME_LENGTH]
        == 71 + tctf.ICD.Sizes.PRIMARY_HEADER_OCTETS.value - 1
    )  # Per ICD subtract 1
    assert decoded_frame_header[tctf.HeaderKeys.FRAME_SEQ_NUM] == frame_seq_num
    assert decoded_frame.payload == random_payload_random_len
    assert decoded_frame.ecf == None
