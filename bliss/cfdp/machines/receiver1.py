    ...
    def update_state(self, event=None, pdu=None, request=None):
        ...
        if self.state == self.S1:
            ...
            if event == Event.RECEIVED_CANCEL_REQUEST:
                bliss.core.log.info("Receiver {0}: Received CANCEL REQUEST".format(self.transaction.entity_id))
                ...
                bliss.core.log.info("Receiver {0}: Received REPORT REQUEST".format(self.transaction.entity_id))
            ...
            # NON-USER ISSUED
                ...
                bliss.core.log.info("Receiver {0}: Received NOTICE OF CANCELLATION".format(self.transaction.entity_id))
                self.cancel()
                self.finish_transaction()
                self.shutdown()

            elif event == Event.RECEIVED_METADATA_PDU:
                bliss.core.log.info("Receiver {0}: Received METADATA PDU event".format(self.transaction.entity_id))
                assert(pdu)
                ...
                if self.metadata.file_transfer:
                    ...
                    try:
                        ...
                        bliss.core.log.error('Receiver {0} -- could not open file: {1}'
                ...
                bliss.core.log.info("Receiver {0}: Received EOF CANCEL PDU event".format(self.transaction.entity_id))
            ...
            # NON-USER ISSUED
                ...
                bliss.core.log.info("Receiver {0}: Received FILE DATA PDU event".format(self.transaction.entity_id))
                ...
                if self.metadata.file_transfer:
                    ...
                    if self.transaction.recv_file_size != pdu.file_size:
                        bliss.core.log.error('Receiver {0} -- file size fault. Received: {1}; Expected: {2}'
