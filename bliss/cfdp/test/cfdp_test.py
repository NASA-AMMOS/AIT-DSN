...
import bliss.core
from bliss.cfdp.cfdp import CFDP
from bliss.cfdp.machines import Sender1
...
class CFDPCommandTest(unittest.TestCase):
    ...
    def test_cmd_invalid_tx(self):
        invalid_tx = 5
        import bliss.cfdp.exceptions
        with self.assertRaises(bliss.cfdp.exceptions.InvalidTransaction):
    ...
    def test_suspend(self):
        ...
        import bliss.cfdp.timer
        if self.machine.inactivity_timer:
            self.assertEqual(self.machine.inactivity_timer.timer_mode, bliss.cfdp.timer.TimerMode.TIMER_PAUSED,
                            "Inactivity timer is paused")
        if self.machine.ack_timer:
            self.assertEqual(self.machine.ack_timer.timer_mode, bliss.cfdp.timer.TimerMode.TIMER_PAUSED,
                             "Ack timer is paused")
        if self.machine.nak_timer:
            self.assertEqual(self.machine.inactivity_timer.nak_timer, bliss.cfdp.timer.TimerMode.TIMER_PAUSED,
                             "Nak timer is paused")
